...
class PollingManager:
    ...
    def adaptive_polling(self, 
                        config: PollingConfig,
                        min_interval: int = 1,
                        max_interval: int = 60) -> Dict[str, Any]:
        """
        Adaptive polling that adjusts interval based on data availability
        
        Args:
            config: Base polling configuration
        ...
        Returns:
            Polling results and statistics
        """
        logger.info(f"Starting adaptive polling (interval range: {min_interval}-{max_interval} minutes)")
        
        if config.symbols is None:
            config.symbols = ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA"]  # Default symbols for adaptive polling
        ...
        try:
            while True:
                ...
                logger.info(f"=== Adaptive Polling Cycle {iteration} at {cycle_start} (interval: {current_interval} min) ===")
                ...
                if cycle_records > 0:
                    successful_cycles += 1
                    # Decrease interval if we got data (more frequent polling)
                    current_interval = max(min_interval, current_interval - 1)
                    logger.info(f"Cycle {iteration} successful: {cycle_records} records, decreasing interval to {current_interval} min")
                else:
                    failed_cycles += 1
                    # Increase interval if no data (less frequent polling)
                    current_interval = min(max_interval, current_interval + 2)
                    logger.warning(f"Cycle {iteration} failed: no records, increasing interval to {current_interval} min")
                
                # Wait for next cycle with adaptive interval
                time.sleep(current_interval * 60)
                
        except KeyboardInterrupt:
            logger.info("Adaptive polling interrupted by user")
        except Exception as e:
